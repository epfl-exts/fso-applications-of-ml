    ...
    while train_size is None:
        ...
        try:
            ...
            if not 100 <= train_size <= max_length:
                train_size = None
                print(
                    "Please choose a number between 100 and {max}".format(
                        max=max_length
                    )
                )
        except ValueError:
...
def generate_training_data(
    data_full, labels_full, training_samples=10000, training_contamination=0.25
):
    X = data_full
    y = labels_full
    ...
    if n_attack > 0:
        _, X_n, _, y_n = train_test_split(
            X_normal, y_normal, test_size=n_normal, random_state=7
        )
        _, X_a, _, y_a = train_test_split(
            X_attack, y_attack, test_size=n_attack, random_state=7
        )
        data_train = pd.concat([X_n, X_a])
        labels_train = pd.concat([y_n, y_a])
    else:
        _, data_train, _, labels_train = train_test_split(
            X_normal, y_normal, test_size=n_normal, random_state=42
        )
...
def create_dataset():
    ...
    data_train, labels_train = generate_training_data(
        data_full,
        labels_full,
...
def visualise_training_data(dataset, max_plot=1000):
    ...
    if data.shape[0] > max_plot:
        data, labels = downsample_for_plot(data, labels, size=max_plot)
...
def build_anomaly_detector(dataset, expected_contamination=0.1, with_PCA=False):

    # Load training set
    # Need to convert to_numpy, otherwise get:
    # UserWarning: X does not have valid feature names, but IsolationForest was fitted with feature names
    X_train = dataset["data_train"].to_numpy()
    y_train = dataset["labels_train"]

    # Define X_test, y_test
    X_test = dataset["data_test1"].to_numpy()
...
def make_predictions(dataset, clf):
    ...
    if isinstance(X_test,np.ndarray):
        ...
        X_test = dataset["data_test1_PCA"].to_numpy()
    #X_test = dataset["data_test1_PCA"].to_numpy()
    y_test = dataset["labels_test1"]  # ["attack"]
...
def evaluate_model(dataset, clf):
    ...
    print(
        "Attacks identified correctly (true positive):",
        cm[1, 1],
        " of ",
        cm[1, 1] + cm[1, 0],
        "(",
...
def detailed_evaluation(dataset, clf, colour_by_level=2, perplexity=25, max_plot=1000):
    ...
    if data.shape[0] > max_plot:
        data, labels = downsample_for_plot(data, labels, size=1000)
    ...
    sns.scatterplot(
        ...
        marker="^",
        s=70,
        palette=color_dict_errors,
        legend=False,
    )
    plt.title("Triangles mark missed predictions. Dark blue triangles are false alarms.")
    plt.legend()
    plt.show()